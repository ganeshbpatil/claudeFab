...
const revealEls = document.querySelectorAll(
  '.overview__text, .overview__visual, .amenity-card, ' +
  '.highlights__img-wrap, .highlights__content, ' +
  '.contact__info, .contact__form-wrap, .gallery .container'
);

revealEls.forEach((el, i) => {
  el.classList.add('reveal');
  if (i % 6 === 1) el.classList.add('reveal-delay-1');
  if (i % 6 === 2) el.classList.add('reveal-delay-2');
  if (i % 6 === 3) el.classList.add('reveal-delay-3');
});
...
// ── Amenity cards staggered reveal ────────────────────────
...
window.addEventListener('scroll', () => {
  ...
  if (y < window.innerHeight && heroImg) {
    heroImg.style.transform = `scale(1.0) translateY(${y * 0.25}px)`;
  }
}, { passive: true });
